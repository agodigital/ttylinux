...
[[ $# != 1 ]] && {
echo "This script take one argument, the top-level ttylinux-ppc ISO directory."
echo "Usage: qemu-powerpc.sh <directory>"
exit 1
}
...
_path=""
for p in ${PATH//:/ }; do
	if [[ -x $p/qemu-system-ppc ]]; then _path=$p/qemu-system-ppc; fi
done
if [[ x"${_path}" = x ]]; then
	echo ""
	echo "Cannot find an executable \"qemu-system-ppc\" program in your"
	echo "\$PATH setting.  Maybe you need to set your \$PATH or download"
	echo "and install qemu.  Qemu can be found at http://wiki.qemu.org/"
	echo ""
	exit 1
fi
unset _path
