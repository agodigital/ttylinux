...
# Get library ports parts and make manifest entries.
...
#
cd ${XBT_LIBC}
for p in ${XBT_SCRIPT_DIR}/glibc/${XBT_LIBC}-*.patch; do
	if [[ -f "${p}" ]]; then
		patch -Np1 -i "${p}"
		cp "${p}" "${XBT_XSRC_DIR}"
		_p="$(basename ${p})"
...
xbt_debug_break "configured ${XBT_LIBC} for stage 1" >&${CONSOLE_FD}
...
xbt_debug_break "configured ${XBT_LIBC} for stage 2" >&${CONSOLE_FD}
...
xbt_debug_break "maked ${XBT_LIBC} for stage 2" >&${CONSOLE_FD}
